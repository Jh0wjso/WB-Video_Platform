import "../styles.css";
import SectionButtons from "../../SectionButtons";
import SectionLinks from "../../SectionLinks";
import NewSectionLinks from "../../NewSectionLinks";

export default function SeasonOneBbz() {
  return (
    <div className="containerEps">
      <h1 className="text-center text-4xl font-bold">Saga dos Sayajins</h1>
      <section className="mt-8 flex flex-col items-center">
        <SectionButtons 
          num1="1" url1="1PwHTtguciWb7A0_e-OP-_vVK3Xbc8jae" name1="Surge um mini Goku! Seu nome é Gohan"
          num2="2" url2="1PuUzaZQSZwd4f_lgT_jXetbGo3mWK9Yu" name2="Um guerreiro muito poderoso com antecedentes"
          num3="3" url3="1Pud1YkdkqkQkHPSa_HE5oPojqzlkXwma" name3="A combinação mais forte deste mundo"
          num4="4" url4="1PnGwK6s6Cb6mlsKDyOAxNyIORRaNrO7O" name4="Piccolo usa sua melhor estratégia! Gohan é um garoto"
          num5="5" url5="1PoSJeNm-SdViDLp5LUmapoQ6BPYsQ8LX" name5="Goku morre! Só existe uma chance"
          num6="6" url6="1PnFEAcMIwCxZVQFm4YztFDCZ_-9owfJX" name6="Enma Samá também se surpreende! Terá que lutar"
        />
        <SectionButtons 
          num1="7" url1="1QDJgaT_V_EkWT4ko36xkASke5qGp_l6A" name1="Terá que sobreviver aos dinossauros! O difícil"
          num2="8" url2="1Q3GS4aPLQ5KcwyiCoXJrFjmK3CIKxmO-" name2="Uma transformação por causa da lua"
          num3="9" url3="1Q4dMMIcVM8P3I56oqzsi2OIl4fQHg3E1" name3="Perdão Sr. Robô! Lágrimas que desaparecem no deserto"
          num4="10" url4="1QSElWhGexDLTRiuXbyEvR7aXWkm7ga4R" name4="Não chore Gohan! É a primeira batalha"
          num5="11" url5="1QKg9mrZiXc-nCd4-w81FaCP6pnbU_kim" name5="Os guerreiros mais fortes do universo"
          num6="12" url6="1QMD_p-Pv21qVWEGYS68XUXSJq2ZcweGS" name6="Um pequeno descanso! Goku cai do caminho da serpente"
        />
        <SectionButtons 
          num1="13" url1="1R4kMnkdyAgCsMoNRj1ln9H1pw6UZHkmo" name1="Não toque nisso! A fruta secreta de Enma Sama"
          num2="14" url2="1QmW4n50qH4qXFZw8mirlghCQ1d57irte" name2="Uma doce tentação! A hospitalidade da princesa serpente"
          num3="15" url3="1Qy4PRE80aFDmWF_mKoPTUYr5HNtiQGbf" name3="Fugindo de Piccolo! Gohan enfrenta uma tempestade"
          num4="16" url4="1QwG0fEUsmGsNLf345b7ZpoPLV8ue01r1" name4="Corre Gohan! Chi-Chi o espera na Montanha Paozu"
          num5="17" url5="1QpghbsnrZ3jNQYiBa0bJxoSHalXllHGo" name5="Uma cidade sem futuro! Um longo caminho até a vitória"
          num6="18" url6="1QiWmI8kjTweX3qA4olHxPEd1IoF9l6ZK" name6="O final do caminho da serpente! É o Sr. Kaio"
        />
        <SectionButtons 
          num1="19" url1="12_GuGnyAB1EM0o1pOqi6unWOic104RWz" name1="Domine a força da gravidade! Agarre Bubb"
          num2="20" url2="1sbMS68U-BQpib6rGpS5_WeUifhWvaR6u" name2="A lenda revive! Os antepassados de Goku"
          num3="21" url3="1jZmjv7ceRokclzdNW6uBnsNQwVG0IgiZ" name3="Sai daí Shen-Long! Os Sayajins chegaram à terra"
          num4="22" url4="1eRdXH-gHVFAw2SLU2VYlrZrFu4H-y3Vi" name4="Impossível! Saibaimans nascidos da terra"
          num5="23" url5="1VG9vBSnqF4sIxLj245Ok1FHkQ9YdXMzX" name5="Yamcha morre! Os temíveis Saibaimans"
          num6="24" url6="1kY8ZgrMRYArpvS8iHZ0MVAUuFywb3N3e" name6="Adeus Ten-Shin! A arte do sacrifício de Chaos"
        />

        <SectionButtons 
          num1="25" url1="1w94RUJI1MP25VxSeXAFl3lBlCh_QSlLd" name1="O sacrifício de Tenshinhan! O último Kikiuhouda"
          num2="26" url2="1wGqczccxCB2htPoV7nCWZvMCNKeEGhj0" name2="Só restam três horas de vida na terra! Apresse-se Goku"
          num3="27" url3="1WXbFSpk7iLONP-gBhksyeECUIdidKyy2" name3="Explode a fúria de Gohan"
          num4="28" url4="15O3JzfF2A6FRDvXO-WvLGJxtZMAQWxnt" name4="O ataque dos Sayajins! Kami-Sama e Piccolo morrem"
          num5="29" url5="1VOaeqrciR3KuXWwAfGtvdlE1qehHEBpz" name5="Papai você é incrível! Uma técnica mortal chamada Kaioken"
          num6="30" url6="16iGpZVM_AcF1HFX5exrRb36axSpzVD1B" name6="Uma luta além dos limites! Goku contra Vegeta"
        />
        <section className="episodeButtons">
          <SectionLinks num="31" url="1UBriUxQBPUnIozM5kC-5PJvMAtJJIU8-" name="Arrisque a vida Goku! A última opção" />
          <SectionLinks num="32" url="1shakrpru_HawgVL2yjJCeIm2z24jl2QD" name="Seu poder de luta aumentou dez vezes" />
          <SectionLinks num="33" url="13baU7nOcr8a3MQopOR1KC-Zyv1LmSg4p" name="Não morra pai!! O poder oculto de Gohan" />
          <SectionLinks num="34" url="12gdIveLOSY242i4k5KE85Of37EUD0CQC" name="Pegue-o Kuririn! Uma Genki-Dama cheia de esperança" />
          <SectionLinks num="35" url="1y-peVZo8bBxS2MFv0NPBRwGEVrLw_Ikx" name="Ocorre um milagre! A transformação de Gohan" />
        </section>
        <NewSectionLinks num="1" name="dragon-ball-z-dublado" />
      </section>
    </div>
  );
}